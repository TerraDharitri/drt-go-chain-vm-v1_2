...
func TestDelegation_v0_1_latest_full(t *testing.T) {
	if testing.Short() {
		t.Skip("not a short test")
	}

	runAllTestsInFolder(t, "delegation/v0_1_latest_full")
}
...
func runSingleTest(t *testing.T, folder string, filename string) { //nolint:unused
	executor, err := am.NewVMTestExecutor("../../scenarioexec")
	require.Nil(t, err)
	runner := mc.NewScenarioRunner(
		executor,
